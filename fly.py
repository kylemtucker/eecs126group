...
"""
def get_turns_for_resources(player_id, board, resources, required):
...
MAX_POINTS - POINTS victory cards.
"""
def get_turns_for_cards(player_id, board, points, resources):
    if points >= MAX_POINTS:
        return 0
    return get_turns_for_resources(player_id, board, resources, (MAX_POINTS - points) * COSTS[CARD])
...
def get_first_settlement_goal(player_id, board):
    ...
    def num_turns(settlement):
        ...
        result = get_turns_for_cards(player_id, board, 0, np.array([1, 2, 2]))
...
"""
def get_next_settlement_goal(player_id, board, points, resources):
    ...
    for settlement in range(board.max_vertex):
        ...
        if end not in invalid_settlement_locations:
            ...
            if num_roads not in settlement_costs:
                ...
                settlement_costs[num_roads] = get_turns_for_resources(player_id, board, resources, required)
            num_turns = settlement_costs[num_roads]
            #Change state to compute hitting time, then revert state.
            board.settlements[settlement] = player_id
            num_turns += get_turns_for_cards(player_id, board, points, np.zeros(3))
            del board.settlements[settlement]
...
"""
def get_next_city_goal(player_id, board, points, resources):
    ...
    for city in board.get_player_settlements(player_id):
        ...
        num_turns = get_turns_for_cards(player_id, board, points + 1, np.zeros(3))
    ...
    min_num_turns += get_turns_for_resources(player_id, board, resources, COSTS[CITY])
...
"""
Returns a goal for the next action.

The current implementation chooses from the minimum number of turns
of buying cards, get_next_settlement_goal, and get_next_city_goal.
"""
def get_next_goal(player_id, board, points, resources):
    #1) Default goal is to buy cards.
    next_goal = Goal(CARD, num_turns=get_turns_for_cards(player_id, board, points, resources))
    #2) Decide if settlement building is better.
    goal = get_next_settlement_goal(player_id, board, points, resources)
    if goal != None and goal.num_turns < next_goal.num_turns:
        next_goal = goal
    #3) Decide if city building is better.
    goal = get_next_city_goal(player_id, board, points, resources)
    if goal != None and goal.num_turns < next_goal.num_turns:
        next_goal = goal
    return next_goal
...
"""
def action(self):
    while self.points < MAX_POINTS:
        if len(self.preComp) == 0:
            #Generate new goal on the fly.
            goal = get_next_goal(self.player_id, copy.deepcopy(self.board), self.points, self.resources[:])
            self.preComp.append(goal)
            #Add road goals to plan.
            if goal.type == SETTLEMENT:
                start = goal.start
                end = goal.end
                while end != start:
                    if end[0] < start[0]:
...
def planBoard(baseBoard):
    ...
    goal = get_first_settlement_goal(board.num_players, board)
    return [goal]
